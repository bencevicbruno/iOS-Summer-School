...
struct PersonDetailView: View {
    ...
    @State private var conferencePlaceName = "UNKNOWN"
    ...
    var body: some View {
        ZStack {
            ...
            VStack(alignment: .center, spacing: 20) {
                ...
                Text("Conference place: \(conferencePlaceName)")
            }
        }
        .onAppear(perform: {
            let locationService = LocationService()
            
            locationService.getLocationName(person.location) { placeName in
                DispatchQueue.main.async {
                    self.conferencePlaceName = placeName
                }
            }
        })
    }
